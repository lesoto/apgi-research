    ...
    def __init__(self, enable_apgi: bool = True):
        ...
        if self.enable_apgi:
            params = APGIParameters(
                tau_S=float(cast(str, APGI_PARAMS.get("tau_s", 0.35))),
                beta=float(cast(str, APGI_PARAMS.get("beta", 1.5))),
                theta_0=float(cast(str, APGI_PARAMS.get("theta_0", 0.5))),
                alpha=float(cast(str, APGI_PARAMS.get("alpha", 5.5))),
                gamma_M=float(cast(str, APGI_PARAMS.get("gamma_M", -0.3))),
                lambda_S=float(cast(str, APGI_PARAMS.get("lambda_S", 0.1))),
                sigma_S=float(cast(str, APGI_PARAMS.get("sigma_S", 0.05))),
                sigma_theta=float(cast(str, APGI_PARAMS.get("sigma_theta", 0.02))),
                sigma_M=float(cast(str, APGI_PARAMS.get("sigma_M", 0.03))),
                rho=float(cast(str, APGI_PARAMS.get("rho", 0.7))),
                theta_survival=float(cast(str, APGI_PARAMS.get("theta_survival", 0.3))),
                theta_neutral=float(cast(str, APGI_PARAMS.get("theta_neutral", 0.4))),
            )
            ...
            if APGI_PARAMS.get("hierarchical_enabled", True):
                ultimate_params = UltimateAPGIParameters(
                    ...
                    beta_cross=float(cast(str, APGI_PARAMS.get("beta_cross", 0.2))),
                    tau_levels=cast(
                        Optional[List[float]],
            ...
            # 100/100: Precision expectation gap (Pi vs Pi_hat)
            ...
            self.neuromodulators = {
                "ACh": float(cast(str, APGI_PARAMS.get("ACh", 1.0))),
                "NE": float(cast(str, APGI_PARAMS.get("NE", 1.0))),
                "DA": float(cast(str, APGI_PARAMS.get("DA", 1.0))),
                "HT5": float(cast(str, APGI_PARAMS.get("HT5", 1.0))),
            }
