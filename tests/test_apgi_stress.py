...
def test_long_run_memory_and_throughput(benchmark=None):
    ...
    assert len(apgi.dynamics.S_history) <= 10000  # nosec: B101 - Test assertion
    assert len(apgi.dynamics.theta_history) <= 10000  # nosec: B101 - Test assertion
    assert len(apgi.dynamics.M_history) <= 10000  # nosec: B101 - Test assertion
    assert len(apgi.dynamics.ignition_history) <= 10000  # nosec: B101 - Test assertion

    # Assert result size matches batch
    assert len(res["S"]) == n_trials  # nosec: B101 - Test assertion
